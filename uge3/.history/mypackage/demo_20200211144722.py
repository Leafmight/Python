...
def write_list_to_file(output_file, lst):
    ...
    with open(output_file, 'w', newline=newline) as output_file:
        ...
        for line in lst:
            output_writer.writerow({'NAME' : line.name, 'GENDER' : line.gender, 'DATASHEET' : line.data_sheet, 'IMG_URL' : line.image_url})
...
def print_students():
    ...
    for student in read_csv()[1:]:
        for course in student[2]:
            lst_courses.append('name' : course[0], 'classroom' : course[1], 'teacher' : course[2], 'ETCS' : course[3], 'grade' : course[4])
        persons.append({'name': student[0], 'gender' : student[1], 'dataSheet' : student[2], 'img_url': student[3]})
    return persons
