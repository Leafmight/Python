...
class Student():
    ...
    def __repr__(self):
        return 'Course(%r, %r, %r, %r)' % (self.name, self.gender, self.data_sheet, self.image_url)
    
    def __str__(self):
        return 'stud_name: {name} {dSheet} img_url: {img}'.format(
            name=self.name, dSheet=self.data_sheet, img=self.image_url)
